...
class KTPOCR(object):
    ...
    def extract(self, extracted_result):
        ...
        for word in extracted_result.split("\n"):
            ...
            if "Nama" in word:
                word = word.split(':')
                self.result.nama = word[-1].replace('Nama ','').translate(replace_table)
                continue
            ...
            if 'Darah' in word:
                ...
                try:
                    self.result.golongan_darah = re.search("(O|A|B|AB)", word[-1])[0].translate(replace_table)
                except:
                    self.result.golongan_darah = '-'

            if 'Alamat' in word:
                self.result.alamat = self.word_to_number_converter(word).replace("Alamat ","").translate(replace_table)

            if 'NO.' in word:
                self.result.alamat = self.result.alamat + ' '+word.translate(replace_table)

            if "Kecamatan" in word:
                if len(word.split(':')) > 1:
                    self.result.kecamatan = word.split(':')[1].strip().translate(replace_table)
            ...
            if 'Kewarganegaraan' in word:
                if len(word.split(':')) > 1:
                    self.result.kewarganegaraan = word.split(':')[1].strip().translate(replace_table)
            ...
            if 'Agama' in word:
                self.result.agama = word.replace('Agama',"").strip().translate(replace_table)

            if 'Perkawinan' in word:
                self.result.status_perkawinan = word.split(':')[1].translate(replace_table)

            if "RTRW" in word:
                word = word.replace("RTRW",'')
                if len(word.split('/')) > 1:
                    self.result.rt = word.split('/')[0].strip().translate(replace_table)
                    self.result.rw = word.split('/')[1].strip().translate(replace_table)
